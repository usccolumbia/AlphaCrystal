...
"""
# Usage:
# to calculate rmsd,mae,rms and accuracy
# $python rmsd_mae_rms_accuracy.py --cif 2-14-mp-236.cif --predicted 2-14-mp-236_predicted.cif
...
"""
"""
D:\lib\site-packages\pymatgen\io\cif.py
330 line add code:self.cos=[]
in def _unique_coords method(583 line):
before return coords, magmoms add code：self.cos.append(coords)
before return coords, [Magmom(0)] * len(coords) add code：self.cos.append(coords)
"""
...
fp = open(args.predicted.replace('.cif','_rmsd_mae_rms_accuracy'), "w")
fp.truncate()
with open(args.predicted.replace('.cif','_rmsd_mae_rms_accuracy'), "w") as fw:
